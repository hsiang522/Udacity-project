...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing==True: 
            ...
            #self.epsilon = math.exp(-0.5 * self.test)
            self.epsilon = math.pow(self.ace, self.test)
    ...
    def build_state(self):
        ...
        #state = waypoint + "_" + str(inputs['light'])+ "_" + str(inputs['oncoming'])\
                #+ "_" + str(inputs['left'])+ "_" + str(inputs['right'])
    ...
    def choose_action(self, state):
        ...
        maxQ_action = max(Q_state,key=Q_state.get)
        '''
        
        Q_state = self.Q[state]
        maxvalue_Q_state = max(Q_state.values())
        maxQ_action = random.choice([k for (k, v) in Q_state.items() if v == maxvalue_Q_state])
